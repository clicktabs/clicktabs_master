...
#employee_patient .dataTables_scroll,
#employee_patient .dataTables_scrollHead,
.ui-tabs-panel .dataTables_scroll,
.ui-tabs-panel .dataTables_scrollHead {
    padding-top: 0;
}
.patient_info .patient_info_wrapper {padding-bottom: 20px;}
.ui-tabs .dataTable, .ui-tabs .dataTables_scrollHeadInner {
    width: 100% !important;
}
</style>
@endsection
...
@if(!$user->hasRole('super-admin') || $current_user)
                                    ...
                                    <div class="patient_info_content">
                                        ...
                                        <div
                                            ...
                                            <table>
                                                ...
                                                <tr>
                                                    <td>Employee ID:</td>
                                                    <td>{{!empty($current_user->employee_id)?$current_user->employee_id:''}}</td>
                                                </tr>
                                                <tr>
                                                    <td>Birthday:</td>
                                                    <td>{{ !empty($current_user->dob) ? \Carbon\Carbon::parse($current_user->dob)->format('M d, Y') :''}}</td>
                                                </tr>
                                                <tr>
                                                    <td>Hire Date:</td>
                                                    <td>{{ !empty($current_user->doh) ? \Carbon\Carbon::parse($current_user->doh)->format('M d, Y'): '' }}</td>
                                                </tr>
                                                <tr>
                                                    <td>Employment Status:</td>
                                                    <td>@if(!empty($current_user->status) && $current_user->status == 1)
                                                            employee
                                                        @elseif(!empty($current_user->status) && $current_user->status != 1)
                                                            ex employee
                                                        @else
    ...
    <div id="employee_info_tabs" class="mt-[0px] bg-[#fff] p-[0px]">
        <ul class="d-flex flex-nowrap align-items-center gap-3  py-[10px] px-[20px] mb-[15px] bg-[#4133BF] " >
        ...
        <div id="mytask">
            <div class="bg-white bd rounded-sm border border-slate-200 mt-2">
                <div class="dk">
                    <table class="table client_table ux ou display nowrap" id="schedulingList21">
                        <thead class="bg-[#4133BF] text-[#fff]">
                            <tr>
                                <th>Patient Name</th>
                                <th>Task</th>
