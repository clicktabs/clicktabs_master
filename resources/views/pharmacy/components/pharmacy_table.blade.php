            ...
            <tr>
                <td class="dx">
                    <div class="flex items-center">
                        ...
                        <div class="text-slate-800 font-weight-bolder" style="font-weight: 700;">{{ $pharmacy->name }}</div>
                    </div>
                </td>
                <td class="dx">
                ...
                <td class="dx">
                    <div>{{ $pharmacy->telephone }}</div>
                </td>
                <td class="dx">
                    <div>{{ $pharmacy->fax }}</div>
                </td>
                <td class="dx">
                    <div>{{ $pharmacy->reference_code }}</div>
                </td>
                <td class="dx">
                    <div>
                ...
                <td class="dx">
                    <a href="{{ route('pharmacy.edit', encrypt($pharmacy->id)) }}" class="btn ho xi ye">Edit</a>
                </td>
                <td class="dx">
                    <form action="{{ route('change_pharmacy_status') }}" method="post">
                        @csrf
                        <input type="hidden" name="pharmacyId" value="{{ encrypt($pharmacy->id) }}">
                        @if ($pharmacy->active_status)
