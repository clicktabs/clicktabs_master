<table class="ux ou">
    ...
    <thead class="go gv gq hp rounded-sm">
        <tr>
            ...
            <th class="dx">
                <div class="gh gn">Reference Code</div>
            </th>
            <th class="dx">
    ...
    <tbody class="text-sm gp le ln">
        <!-- Row -->
        @if ($addonTaxes)
            @foreach ($addonTaxes as $addonTax)
            <tr>
                <td class="dx">
                    <div class="flex items-center">
                        ...
                        <div class="text-slate-800 font-weight-bolder" style="font-weight: 700;">{{ $addonTax->name }}</div>
                    </div>
                </td>
                <td class="dx">
                    <div class="gn">{{ $addonTax->reference_code }}</div>
                </td>
                <td class="dx">
                    <div class="gn">
                        @if ($addonTax->active_status)
                ...
                <td class="dx">
                    <a href="{{ route('taxes.edit', encrypt($addonTax->id)) }}" class="btn ho xi ye">Edit</a>
                </td>
                <td class="dx">
                    <form action="{{ route('change_tax_status') }}" method="post">
                        @csrf
                        <input type="hidden" name="tax_id" value="{{ encrypt($addonTax->id) }}">
                        @if ($addonTax->active_status)
