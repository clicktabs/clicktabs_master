<table class="ux ou">
    ...
    <thead class="go gv gq hp rounded-sm">
        <tr>
            ...
            <th class="dx">
                <div class="gh gn">Description</div>
            </th>
            <th class="dx">
                <div class="gh gn">Privileges</div>
            </th>
            <th class="dx">
            ...
            <tr>
                <td class="dx">
                    <div class="flex items-center">
                        ...
                        <div class="text-slate-800 font-weight-bolder" style="font-weight: 700;">{{ $leaveSub->name }}</div>
                    </div>
                </td>
                <td class="dx">
                    <div class="gn">{{ $leaveSub->reference_code }}</div>
                </td>
                <td class="dx">
                    <div class="gn">{{ $leaveSub->description }}</div>
                </td>
                <td class="dx">
                    <div class="gn">{{ $leaveSub->leave_privileges }}</div>
                </td>
                <td class="dx">
                    <div class="gn">
                        @if ($leaveSub->active_status)
                ...
                <td class="dx">
                    <a href="{{ route('leaveGroups.edit', encrypt($leaveSub->id)) }}" class="btn ho xi ye">Edit</a>
                </td>
                <td class="dx">
                    <form action="{{ route('change_leave_status') }}" method="post">
                        @csrf
                        <input type="hidden" name="leave_group_id" value="{{ encrypt($leaveSub->id) }}">
                        @if ($leaveSub->active_status)
