            ...
            <tr>
                <td class="dx">
                    <div class="flex items-center">
                        ...
                        <div class="text-slate-800 font-weight-bolder" style="font-weight: 700;">{{ $addonService->name }}</div>
                    </div>
                </td>
                <td class="dx">
                    <div class="gn">{{ $addonService->reference_code }}</div>
                </td>
                <td class="dx">
                    <div class="gn">
                        @if ($addonService->active_status)
                ...
                <td class="dx">
                    <a href="{{ route('serviceRequired.edit', encrypt($addonService->id)) }}" class="btn ho xi ye">Edit</a>
                </td>
                <td class="dx">
                    <form action="{{ route('change_service_required_status') }}" method="post">
                        @csrf
                        <input type="hidden" name="serviceId" value="{{ encrypt($addonService->id) }}">
                        @if ($addonService->active_status)
