<table class="ux ou">
    ...
    <tbody class="text-sm gp le ln">
        <!-- Row -->
        @if ($addonSecondaryInsurances)
            @foreach ($addonSecondaryInsurances as $addonSecondaryInsurance)
            <tr>
                <td class="dx">
                    <div class="flex items-center">
                        ...
                        <div class="text-slate-800 font-weight-bolder" style="font-weight: 700;">{{ $addonSecondaryInsurance->name }}</div>
                    </div>
                </td>
                <td class="dx">
                    <div class="gn">{{ $addonSecondaryInsurance->reference_code }}</div>
                </td>
                <td class="dx">
                    <div class="gn">
                        @if ($addonSecondaryInsurance->active_status)
                ...
                <td class="dx">
                    <a href="{{ route('secondaryInsurance.edit', encrypt($addonSecondaryInsurance->id)) }}" class="btn ho xi ye">Edit</a>
                </td>
                <td class="dx">
                    <form action="{{ route('change_secondary_insurance_status') }}" method="post">
                        @csrf
                        <input type="hidden" name="secondaryInsuranceId" value="{{ encrypt($addonSecondaryInsurance->id) }}">
                        @if ($addonSecondaryInsurance->active_status)
