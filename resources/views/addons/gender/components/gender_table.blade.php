<table class="ux ou">
    ...
    <tbody class="text-sm gp le ln">
        <!-- Row -->
        @if ($addonGenders)
            @foreach ($addonGenders as $addonGender)
            <tr>
                <td class="dx">
                    <div class="flex items-center">
                        ...
                        <div class="text-slate-800 font-weight-bolder" style="font-weight: 700;">{{ $addonGender->name }}</div>
                    </div>
                </td>
                <td class="dx">
                    <div class="gn">{{ $addonGender->reference_code }}</div>
                </td>
                <td class="dx">
                    <div class="gn">
                        @if ($addonGender->active_status)
                ...
                <td class="dx">
                    <a href="{{ route('gender.edit', encrypt($addonGender->id)) }}" class="btn ho xi ye">Edit</a>
                </td>
                <td class="dx">
                    <form action="{{ route('change_gender_status') }}" method="post">
                        @csrf
                        <input type="hidden" name="genderId" value="{{ encrypt($addonGender->id) }}">
                        @if ($addonGender->active_status)
