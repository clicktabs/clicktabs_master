            ...
            <tr>
                <td class="dx">
                    <div class="flex items-center">
                        ...
                        <div class="text-slate-800 font-weight-bolder" style="font-weight: 700;">{{ $jobTitle->job_title }}</div>
                    </div>
                </td>
                <td class="dx">
                    <div class="gn">{{ $jobTitle->reference_code }}</div>
                </td>
                <td class="dx">
                    <div class="gn">
                        @if ($jobTitle->active_status)
                ...
                <td class="dx">
                    <a href="{{ route('jobTitles.edit', encrypt($jobTitle->id)) }}" class="btn ho xi ye">Edit</a>
                </td>
                <td class="dx">
                    <form action="{{ route('change_job_status') }}" method="post">
                        @csrf
                        <input type="hidden" name="job_id" value="{{ encrypt($jobTitle->id) }}">
                        @if ($jobTitle->active_status)
