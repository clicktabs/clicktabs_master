<table class="ux ou">
    ...
    <tbody class="text-sm gp le ln">
        <!-- Row -->
        @if ($addonInternalReferralSources)
            @foreach ($addonInternalReferralSources as $addonInternalReferralSource)
            <tr>
                <td class="dx">
                    <div class="flex items-center">
                        ...
                        <div class="text-slate-800 font-weight-bolder" style="font-weight: 700;">{{ $addonInternalReferralSource->name }}</div>
                    </div>
                </td>
                <td class="dx">
                    <div class="gn">{{ $addonInternalReferralSource->reference_code }}</div>
                </td>
                <td class="dx">
                    <div class="gn">
                        @if ($addonInternalReferralSource->active_status)
                ...
                <td class="dx">
                    <a href="{{ route('internalReferralSource.edit', encrypt($addonInternalReferralSource->id)) }}" class="btn ho xi ye">Edit</a>
                </td>
                <td class="dx">
                    <form action="{{ route('change_internal_referral_status') }}" method="post">
                        @csrf
                        <input type="hidden" name="addonInternalReferralSourceId" value="{{ encrypt($addonInternalReferralSource->id) }}">
                        @if ($addonInternalReferralSource->active_status)
