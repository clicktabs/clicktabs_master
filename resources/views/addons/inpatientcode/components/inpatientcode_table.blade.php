<table class="ux ou">
    ...
    <tbody class="text-sm gp le ln">
        <!-- Row -->
        @if ($addonInPatientAdmissionCodes)
            @foreach ($addonInPatientAdmissionCodes as $addonInPatientAdmissionCode)
            <tr>
                <td class="dx">
                    <div class="flex items-center">
                        ...
                        <div class="text-slate-800 font-weight-bolder" style="font-weight: 700;">{{ $addonInPatientAdmissionCode->name }}</div>
                    </div>
                </td>
                <td class="dx">
                    <div class="gn">{{ $addonInPatientAdmissionCode->reference_code }}</div>
                </td>
                <td class="dx">
                    <div class="gn">
                        @if ($addonInPatientAdmissionCode->active_status)
                ...
                <td class="dx">
                    <a href="{{ route('inPatientCode.edit', encrypt($addonInPatientAdmissionCode->id)) }}" class="btn ho xi ye">Edit</a>
                </td>
                <td class="dx">
                    <form action="{{ route('change_patientcode_status') }}" method="post">
                        @csrf
                        <input type="hidden" name="inPatientCodeId" value="{{ encrypt($addonInPatientAdmissionCode->id) }}">
                        @if ($addonInPatientAdmissionCode->active_status)
