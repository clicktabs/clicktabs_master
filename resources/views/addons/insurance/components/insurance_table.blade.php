<table class="ux ou">
    ...
    <tbody class="text-sm gp le ln">
        <!-- Row -->
        @if ($addonPrimaryInsurances)
            @foreach ($addonPrimaryInsurances as $addonPrimaryInsurance)
            <tr>
                <td class="dx">
                    <div class="flex items-center">
                        ...
                        <div class="text-slate-800 font-weight-bolder" style="font-weight: 700;">{{ $addonPrimaryInsurance->name }}</div>
                    </div>
                </td>
                <td class="dx">
                    <div class="gn">{{ $addonPrimaryInsurance->reference_code }}</div>
                </td>
                <td class="dx">
                    <div class="gn">
                        @if ($addonPrimaryInsurance->active_status)
                ...
                <td class="dx">
                    <a href="{{ route('insurance.edit', encrypt($addonPrimaryInsurance->id)) }}" class="btn ho xi ye">Edit</a>
                </td>
                <td class="dx">
                    <form action="{{ route('change_insurance_status') }}" method="post">
                        @csrf
                        <input type="hidden" name="primaryInsuranceId" value="{{ encrypt($addonPrimaryInsurance->id) }}">
                        @if ($addonPrimaryInsurance->active_status)
