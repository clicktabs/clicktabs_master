<table class="ux ou">
    ...
    <tbody class="text-sm gp le ln">
        <!-- Row -->
        @if ($addonAgencyBranches)
            @foreach ($addonAgencyBranches as $addonAgencyBranch)
            <tr>
                <td class="dx">
                    <div class="flex items-center">
                        ...
                        <div class="text-slate-800 font-weight-bolder" style="font-weight: 700;">{{ $addonAgencyBranch->name }}</div>
                    </div>
                </td>
                <td class="dx">
                    <div class="gn">{{ $addonAgencyBranch->reference_code }}</div>
                </td>
                <td class="dx">
                    <div class="gn">
                        @if ($addonAgencyBranch->active_status)
                ...
                <td class="dx">
                    <a href="{{ route('agency.edit', encrypt($addonAgencyBranch->id)) }}" class="btn ho xi ye">Edit</a>
                </td>
                <td class="dx">
                    <form action="{{ route('change_agency_status') }}" method="post">
                        @csrf
                        <input type="hidden" name="agencyBranchId" value="{{ encrypt($addonAgencyBranch->id) }}">
                        @if ($addonAgencyBranch->active_status)
