<table class="ux ou">
    ...
    <tbody class="text-sm gp le ln">
        <!-- Row -->
        @if ($addonPhysicians)
            @foreach ($addonPhysicians as $addonPhysician)
            <tr>
                <td class="dx">
                    <div class="flex items-center">
                        ...
                        <div class="text-slate-800 font-weight-bolder" style="font-weight: 700;">{{ $addonPhysician->name }}</div>
                    </div>
                </td>
                <td class="dx">
                    <div class="gn">{{ $addonPhysician->reference_code }}</div>
                </td>
                <td class="dx">
                    <div class="gn">
                        @if ($addonPhysician->active_status)
                ...
                <td class="dx">
                    <a href="{{ route('physician.edit', encrypt($addonPhysician->id)) }}" class="btn ho xi ye">Edit</a>
                </td>
                <td class="dx">
                    <form action="{{ route('change_physician_status') }}" method="post">
                        @csrf
                        <input type="hidden" name="physicianId" value="{{ encrypt($addonPhysician->id) }}">
                        @if ($addonPhysician->active_status)
