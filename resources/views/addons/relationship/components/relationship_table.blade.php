<table class="ux ou">
    ...
    <tbody class="text-sm gp le ln">
        <!-- Row -->
        @if ($addonRelationships)
            @foreach ($addonRelationships as $addonRelationship)
            <tr>
                <td class="dx">
                    <div class="flex items-center">
                        ...
                        <div class="text-slate-800 font-weight-bolder" style="font-weight: 700;">{{ $addonRelationship->name }}</div>
                    </div>
                </td>
                <td class="dx">
                    <div class="gn">{{ $addonRelationship->reference_code }}</div>
                </td>
                <td class="dx">
                    <div class="gn">
                        @if ($addonRelationship->active_status)
                ...
                <td class="dx">
                    <a href="{{ route('relationship.edit', encrypt($addonRelationship->id)) }}" class="btn ho xi ye">Edit</a>
                </td>
                <td class="dx">
                    <form action="{{ route('change_relationship_status') }}" method="post">
                        @csrf
                        <input type="hidden" name="relationshipId" value="{{ encrypt($addonRelationship->id) }}">
                        @if ($addonRelationship->active_status)
