            ...
            <tr>
                <td class="dx">
                    <div class="flex items-center">
                        ...
                        <div class="text-slate-800 font-weight-bolder" style="font-weight: 700;">{{ $addonSub->race_type }}</div>
                    </div>
                </td>
                <td class="dx">
                    <div class="gn">{{ $addonSub->reference_code }}</div>
                </td>
                <td class="dx">
                    <div class="gn">
                        @if ($addonSub->active_status)
                ...
                <td class="dx">
                    <a href="{{ route('races.edit', encrypt($addonSub->id)) }}" class="btn ho xi ye">Edit</a>
                </td>
                <td class="dx">
                    <form action="{{ route('change_race_status') }}" method="post">
                        @csrf
                        <input type="hidden" name="race_id" value="{{ encrypt($addonSub->id) }}">
                        @if ($addonSub->active_status)
