<form action="{{ route('facilityReferral.store') }}" method="post" class="row">
    @csrf
    <div class="card mb-2">
        ...
        <div class="card-body">
            ...
            @hasrole('super-admin')
                ...
                @else
                    <input id="organisation_id" value={{ $organizations }} type="text" class="form-control s ou" name="organisation_id" value="{{ old('organisation_id') }}" hidden>
                @endif
            @else
                <input id="organisation_id" value={{ $organizations }} type="text" class="form-control s ou" name="organisation_id" value="{{ old('organisation_id') }}" hidden>
            @endhasrole
