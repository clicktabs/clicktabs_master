<table class="ux ou">
    ...
    <tbody class="text-sm gp le ln">
        <!-- Row -->
        @if ($addonFacilityReferrals)
            @foreach ($addonFacilityReferrals as $addonFacilityReferral)
            <tr>
                <td class="dx">
                    <div class="flex items-center">
                        ...
                        <div class="text-slate-800 font-weight-bolder" style="font-weight: 700;">{{ $addonFacilityReferral->name }}</div>
                    </div>
                </td>
                <td class="dx">
                    <div class="gn">{{ $addonFacilityReferral->reference_code }}</div>
                </td>
                <td class="dx">
                    <div class="gn">
                        @if ($addonFacilityReferral->active_status)
                ...
                <td class="dx">
                    <a href="{{ route('facilityReferral.edit', encrypt($addonFacilityReferral->id)) }}" class="btn ho xi ye">Edit</a>
                </td>
                <td class="dx">
                    <form action="{{ route('change_facility_status') }}" method="post">
                        @csrf
                        <input type="hidden" name="facilityReferralId" value="{{ encrypt($addonFacilityReferral->id) }}">
                        @if ($addonFacilityReferral->active_status)
