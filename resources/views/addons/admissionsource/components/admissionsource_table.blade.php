...
{{--            </th>--}}
    ...
    <tbody class="text-sm gp le ln">
        <!-- Row -->
        @if ($addonAdmissionSources)
            @foreach ($addonAdmissionSources as $admissionSource)
            <tr>
                <td class="dx">
                    <div class="flex items-center">
                        ...
                        <div class="text-slate-800 font-weight-bolder" style="font-weight: 700;">{{ $admissionSource->name }}</div>
                    </div>
                </td>
{{--                <td class="dx">--}}
{{--                    <div class="gn">{{ $admissionSource->reference_code }}</div>--}}
{{--                </td>--}}
                <td class="dx">
                    <div class="gn">
                        @if ($admissionSource->active_status)
                ...
                <td class="dx">
                    <a href="{{ route('credentials.edit', encrypt($admissionSource->id)) }}" class="btn ho xi ye">Edit</a>
                </td>
                <td class="dx">
                    <form action="{{ route('change_admission_status') }}" method="post">
                        @csrf
                        <input type="hidden" name="admissionSourceId" value="{{ encrypt($admissionSource->id) }}">
                        @if ($admissionSource->active_status)
